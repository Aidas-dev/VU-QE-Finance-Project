...
def main():
    ...
    # Data export - combined workbook
    os.makedirs(config['output']['xlsx'], exist_ok=True)
    output_path = f"{config['output']['xlsx'] }/Sharpe-ratio-analysis-results.xlsx"
    
    with pd.ExcelWriter(output_path) as writer:
        df.to_excel(writer, sheet_name='Original Price Data')
        clean_df.to_excel(writer, sheet_name='Cleaned Price')
        risk_free_rate.to_excel(writer, sheet_name='Risk Free Rate')
        monthly_returns.to_excel(writer, sheet_name='Monthly Returns')
        sharpe_ratios.to_excel(writer, sheet_name='Sharpe Ratios')
        stats_no_shorting.to_excel(writer, sheet_name='Portfolio Stats (No Shorting)')
        stats_with_shorting.to_excel(writer, sheet_name='Portfolio Stats (With Shorting)')
        optimal_weights_no_shorting.to_excel(writer, sheet_name='Optimal Weights (No Shorting)')
        optimal_weights_with_shorting.to_excel(writer, sheet_name='Optimal Weights (With Shorting)')
